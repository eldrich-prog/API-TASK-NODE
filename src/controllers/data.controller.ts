...
class PostHttpController extends HttpInterface {
    ...
    async post(req:Request, res:Response): Promise<void> {
        req.body.data = String(req.params.str);
        const newPost = await prisma.data.createMany({
            data: req.body
        })
    ...
    async get(req:Request, res:Response) {
        const allPosts = await prisma.data.findMany()
        res.status(200).json(allPosts);
    }

    async put(req:Request, res:Response) {
        const updatedPost = await prisma.data.update({
            where:{
    ...
    async delete(req:Request, res:Response) {
        const deletePost = await prisma.data.delete({
            where:{
                id: parseInt(req.params.id)
            }
        })
